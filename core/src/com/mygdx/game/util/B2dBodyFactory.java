...
public class B2dBodyFactory {
    private static B2dBodyFactory thisInstance;
    private World world;
    ...
    private B2dBodyFactory(World world) {
        this.world = world;
    }

    public static B2dBodyFactory getInstance(World world) {
        if (thisInstance == null) {
            thisInstance = new B2dBodyFactory(world);
        }
        return thisInstance;
    }
...
//        fixedRotation will be true if we want our body to never rotate
        // create a definition
    ...
    public Body makeCirclePolyBody(float posx, float posy, float radius, int material, boolean fixedRotation, float linearX, float linearY) {
//        fixedRotation will be true if we want our body to never rotate
        // create a definition
        BodyDef boxBodyDef = new BodyDef();
        ...
        boxBody.createFixture(makeFixture(material, circleShape));
        circleShape.dispose();
        boxBody.setLinearVelocity(linearX, linearY);
        return boxBody;
    }
